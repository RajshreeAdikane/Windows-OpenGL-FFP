...
GLfloat materialShininess[] = { 164.0f }; //float Vector array if their is value single then we could use only f
...
int initialize(void)
{
	...
	glLightfv(GL_LIGHT0, GL_AMBIENT, lightAmbient); //float Vector array if their is value single then we could use only f
...
void display(void)
{
	...
	gluSphere(quadric, 1.2f, 50, 50); // (quadric object, radius, slices, stacks) vertical subdivin
