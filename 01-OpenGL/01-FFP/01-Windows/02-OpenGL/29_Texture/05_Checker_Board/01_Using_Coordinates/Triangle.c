...
//Checker Boaed related global variables
...
int initialize(void)
{
	...
	glClearColor(0.75f, 0.75f, 0.75f, 1.0f); // state fuc
...
void loadGLTexture(void)
{
	...
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1); // dont do unpacking by any method do by defalut version

	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); // target point, wrap , repeat (mathematical texture is big then wrap in s coordinate and so repeate)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); // target point, wrap , repeat (mathematical texture is big then wrap in t coordinate and so repeate)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); // because we are doing mathematical procedural texturing so we are using GL_NEARESET
	...
	// 0 - mipmap levelnparameter(telling that opengl to do mipmap)
	// GL_RGBA - (Image format) - we have all 4 parameter
	// 0 - Boarder width (0 means we dont have any width)
...
void display(void)
{
	...
	// Bind checkerboard texture
	glBindTexture(GL_TEXTURE_2D, texture_checkerboard);

	glBegin(GL_QUADS);

	// ---------------- LEFT QUAD (Front-facing) ----------------
	...
	// ---------------- RIGHT QUAD (Oblique-facing) ----------------
	// Simulates a 3D angled board on the right
	glTexCoord2f(1.0f, 1.0f);
